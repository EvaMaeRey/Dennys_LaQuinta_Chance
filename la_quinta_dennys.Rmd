...
df=bind_rows(RDennys,RLaQuinta)
df$id=1:nrow(df)
plot(x) ; points(df, col=ifelse(df$chain=="Denny's","red", "blue"), pch="x", cex=.5)
```
...
rbind("DN-LQ"=summary(rdn_lq$dist),
...
d = rbind(data.frame(pair = "DN-LQ (CONUS)", dist = rdn_lq_cus$dist),
...
g2 = ggplot(d, aes(x = dist, color = pair)) + 
  stat_ecdf() + 
  xlab("Distance (km)") +
  ylab("ECDF") + xlim(0,300)#+scale_x_log10()
